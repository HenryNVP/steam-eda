...
```{r}
# Filter for the specified ranges
top_owned_games <- df[df$Estimated.owners %in% c("100000000 - 200000000", 
                                                  "50000000 - 100000000", 
                                                  "20000000 - 50000000"), ]

# Convert Estimated.owners to a numeric range for sorting
top_owned_games$OwnersNumeric <- as.numeric(gsub("[^0-9]", "", sub(" - .*", "", top_owned_games$Estimated.owners))) 

# Sort the filtered games in descending order of owners
sorted_games <- top_owned_games[order(-top_owned_games$OwnersNumeric), ]
...
```{r}
publisher_occurrences <- result %>%
  group_by(Publishers) %>%
  summarise(Occurrences = n()) %>%
  arrange(desc(Occurrences))

# Display the occurrences
print(publisher_occurrences)
```
...
top_publishers_games <- df %>%
  ...
  slice_head(n = 10)  # Get the top 10 publishers
...
ggplot(genre_by_year, aes(x = Year, y = Count, color = Genres)) +
  ...
  labs(title = "Trending Game Genres Over Time", 
       x = "Year", 
...
top_genres_all_time <- df_genres %>%
  group_by(Genres) %>%
  summarise(GameCount = n(), .groups = 'drop') %>%
  arrange(desc(GameCount))  # Sort by the count in descending order

# Print the top 10 genres by count for all time
head(top_genres_all_time, 10)
```
...
# Step 2: Reshape the matrix into long format
cor_long <- as.data.frame(as.table(cor_matrix))

# Rename columns for clarity
colnames(cor_long) <- c("Var1", "Var2", "Correlation")

# Step 3: Plot the correlation heatmap using ggplot2
...
df_rating <- df %>%
  ...
  summarise(
    ...
    mean_positive = mean(Positive, na.rm = TRUE),  # Average Positive rating per game in each group
    mean_negative = mean(Negative, na.rm = TRUE),  # Average Negative rating per game in each group
    median_positive = median(Positive, na.rm = TRUE),  # Median of Positive ratings
    median_negative = median(Negative, na.rm = TRUE),  # Median of Negative ratings
  )
```

```{r}
# Summarize the data by calculating the sum of Positive and Negative ratings
...
```{r}
# Summarize the data by calculating the total positive and negative ratings for each group
df_summary <- df %>%
...
# Calculate total ratings and percentages for positive and negative
...
ggplot(df_summary_long, aes(x = Estimated.owners, y = Percentage, fill = Rating_Type)) +
...
ggplot(df, aes(x = Recommendations, y = Positive)) +
  ...
  labs(title = "Relationship between Positive Reviews and Recommendations",
       x = "Positive Reviews", y = "Recommendations") +
  theme_minimal()
```
...
### 5.1. Hypothesis Testing: Effect of Positive Reviews on Recommendations

Do games with more Positive reviews tend to receive more Recommendations?
- Null Hypothesis ($H_0$): There is no significant relationship between the number of Positive reviews and the number of Recommendations.
- Alternative Hypothesis ($H_1$): *Games with more Positive reviews tend to receive more Recommendations.*

From the graphs in session 4, there appears to be a linear relationship between Positive and Recommendations. Linear regression model is chosen to quantify the relationship between these two variables and test whether it is statistically significant.

```{r}
model1 <- lm(Recommendations ~ Positive, data = df)
summary(model1)
```
Since p-value < 2.2e-16, much less than 0.05, we reject the null hypothesis ($H_0$) and accept the alternative hypothesis ($H_1$). In conclusion, games with more Positive reviews tend to receive more Recommendations.

### 5.2. Hypothesis Testing: Proportion of Positive Reviews by Estimated Owners

Is the proportion of Positive reviews higher for games that have a higher number of Estimated Owners?

- Null Hypothesis ($H_0$): The proportion of Positive reviews is the same across different groups of Estimated Owners.

- Alternative Hypothesis ($H_1$): Games with a higher number of Estimated Owners have a higher proportion of Positive reviews.

```{r}
df_summary <- df_summary %>% filter(Estimated.owners != "0 - 0")  # there is no data in group 0 - 0

contingency_table <- data.frame(
  owner_category = df_summary$Estimated.owners,
  positive_percentage = df_summary$positive_percentage,
  negative_percentage = df_summary$negative_percentage
)

print(contingency_table)

contingency_matrix <- as.matrix(contingency_table[, c("positive_percentage", "negative_percentage")])
```

Chi-squared test is performed to check if positive review proportion varies by Estimated Owners.
...
Since p-value > 0.05, we accept the null hypothesis ($H_0$) and reject the alternative hypothesis ($H_1$). To conclude, *The proportion of Positive reviews is the same across different groups of Estimated Owners.*

### 5.3. Hypothesis Testing: Effect of Multiple Genres on Positive and Negative Reviews

Does the presence of multiple genres in a game affect the number of Positive and Negative reviews?

- Null Hypothesis ($H_0$): The proportion of Positive and Negative reviews is the same regardless of whether a game has one or multiple genres.

- Alternative Hypothesis ($H_1$): Games with multiple genres have a significantly different proportion of Positive and Negative reviews compared to games with a single genre.
...
df$multiple_genres <- as.factor(df$multiple_genres)

# View the updated column
table(df$multiple_genres)
```

Compare number of positive and negative reviews between single and multiple-genre games
```{r}
reviews_by_genre <- df %>%
...
contingency_matrix2 <- as.matrix(reviews_by_genre[, c("positive_percentage", "negative_percentage")])
```

Similarly, Chi-square test is performed.
...
Since p-value < 0.05, we reject the null hypothesis ($H_0$) and accept the alternative hypothesis ($H_1$). *Games with multiple genres have a significantly different proportion of Positive and Negative reviews compared to games with a single genre.*
